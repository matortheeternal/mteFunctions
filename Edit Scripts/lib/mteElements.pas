...
function SearchTListForElement(aElement: IInterface; var tl: TList): Integer;
var
  i: Integer;
begin
  Result := (-1);
  if not Assigned(aElement) then raise Exception.Create('SearchTListForElement: Input Element is not Assigned');
  if not Assigned(tl) then raise Exception.Create('SearchTListForElement: Input List is not Assigned');
  for i := 0 to Pred(tl.Count) do begin
    If Equals(aElement, ObjectToElement(tl.Objects[i]) then begin 
      Result := i;
    end;
  end;
end;
